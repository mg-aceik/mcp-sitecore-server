...
describe("powershell", () => {
    it("security-remove-role-member", async () => {
        ...
        const newRoleArgs: Record<string, any> = {
            identity: roleName,
            description: "Test role created for remove-role-member test"
        };
        ...
        const newUserArgs: Record<string, any> = {
            ...
            fullName: "Test User",
            comment: "Test user created for remove-role-member test"
        };
        ...
        const memberFoundBefore = getRoleMembersJson.Obj.some((member: any) =>
            member.AccountType?.ToString === "User" &&
            member.Name === `sitecore\\${userName}`
        );
        expect(memberFoundBefore).toBe(true);

        // 5. Remove the user from the role using security-remove-role-member
        const removeRoleMemberArgs: Record<string, string> = {
            identity: `sitecore\\${roleName}`,
            members: `sitecore\\${userName}`
        };

        const removeRoleMemberResult = await callTool(client, "security-remove-role-member", removeRoleMemberArgs);

        // 6. Verify the user is no longer a member of the role
        const getRoleMembersAfterResult = await callTool(client, "security-get-role-member", getRoleMemberArgs);
        const getRoleMembersAfterJson = JSON.parse(getRoleMembersAfterResult.content[0].text);

        // Verify the test user is no longer in the role members
        expect(getRoleMembersAfterJson).toMatchObject({});

        // 7. Clean up - Remove user and role
